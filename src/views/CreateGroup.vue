...
export default {
    ...
    methods:{
        submit(){
            console.log(JSON.stringify(this.dataGroup)+window.localStorage.getItem('key'))
             axios.post('http://127.0.0.1:8000/auth/users/me/',this.dataGroup,{ headers: { 'Authorization': `token ${window.localStorage.getItem('token')}` }})
                .then(response => { this.account = response.data.username} )
                .catch(err => {
                    if(err.response){ 
                        this.change()
        ...
        calldata(){
            ...
            axios.get('http://127.0.0.1:8000/auth/users/me/', { headers: { 'Authorization': `token ${window.localStorage.getItem('token')}` }})
